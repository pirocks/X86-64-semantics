...
#RUN cd semantics && /usr/bin/time -v kompile  x86-semantics.k --parse-only --emit-json  --syntax-module X86-SYNTAX  --main-module  X86-SEMANTICS --backend java -I . -I common/x86-config/
RUN mvn -f run_for_every_instruction/pom.xml package
RUN java -jar /home/user/X86-64-semantics/run_for_every_instruction/target/run_for_every_instruction-1.0-SNAPSHOT.jar